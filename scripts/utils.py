...
def adjust_positions(points_dict, min_distance = 0.05):
    keys = list(points_dict.keys())
    points = np.array([points_dict[key] for key in keys])
        ...
        for i in range(len(points)):
            for j in range(i+1, len(points)):
                ...
                if y_distance < min_distance:
                    too_close = True
                    direction = 1 if points[i][1] < points[j][1] else -1
                    move_by = (min_distance - y_distance) / 2
                    points[i][1] -= direction * move_by
                    points[j][1] += direction * move_by
        if not too_close:
            break
        iter += 1
